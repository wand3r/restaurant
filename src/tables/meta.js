...
export const inRoom = R.curry((roomId, tables) =>
  R.pickBy(R.pipe(R.path(["position", "roomId"]), R.equals(roomId)))(tables),
);
export const onCoordinates = R.curry((coordinates, tables) =>
  R.pickBy(R.pipe(toRect, isPointInsideRect(coordinates)))(tables),
);
...
const inRoomOnCoordinates = R.curry((roomId, coordinates, tables) =>
  R.pipe(inRoom(roomId), onCoordinates(coordinates))(tables),
);
...
export const validateDimension = value => {
  return value == undefined
    ? { isValid: false, msg: "Wymiar stołu nie może być pusty" }
    : Number.isNaN(value)
      ? { isValid: false, msg: "Wymiar stołu musi być prawidłową liczbą" }
      : value <= 0
        ? { isValid: false, msg: "Wymiar stołu musi być większy od zera" }
        : value >= 10
          ? {
              isValid: false,
              msg: "Wymier stołu jest musi być mniejszy niż 10 metrów",
            }
          : { isValid: true };
};

export const validateName = ({ id, name }, tables) => {
  return Str.isEmpty(name)
    ? { isValid: false, msg: "Nazwa stołu nie może być pusty" }
    : Str.length(name) > 10
      ? {
          isValid: false,
          msg: "Nazwa stołu nie może być dłuższy niż 10 znaków",
        }
      : R.pipe(Obj.withoutKey(id), Obj.any(x => x.name == name))(tables)
        ? { isValid: false, msg: "Nazwa stół jest już zajęta" }
        : { isValid: true };
};
