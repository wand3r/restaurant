...
import {ReactSVGPanZoom, TOOL_NONE, TOOL_PAN, fitSelection, zoomOnViewerCenter, fitToViewer,setPointOnViewerCenter} from 'react-svg-pan-zoom';
...
const fitSelectionOnCenter = (value, {x,y,width,height}, margin) => {
  const fitted = fitSelection(value, x-margin, y-margin, width+margin, height+margin)
  const centered = setPointOnViewerCenter(fitted, width/2, height/2, fitted.a)
  return centered;
}
...
class Room extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const viewerStateTransform = roomId(this.props) != roomId(nextProps)
  ...
  render() {
    const roomId = roomId(this.props)
    return (
      <Measure onMeasure={({width, height}) => this.setState({componentDimensions: {width,height}})}>
        <div style={{flex: '1', position: 'relative'}}>
          <ReactSVGPanZoom
            ...
            onMouseUp={({x,y}) => roomId && this.props.mouseUp(roomId,{x,y})}
            onMouseMove={e => { }}
            onMouseDown={e => { }}
          ...
          >
            <svg width={0} height={0}>
              <defs>
                <pattern id="img1" patternUnits="userSpaceOnUse" width="203" height="317">
                  <image xlinkHref={wood_pattern} x="0" y="0" width="203" height="317" />
                </pattern>
              </defs>
              <g>
                {Obj.mapToArray(
